...
- **Q-A1 — Phase A scope = ENTIRE SURFACE.** Replicate the full 94-table / 34-function /
  ...
  what the seed framed as Phase C (integration replication); Phase C/D narrow to _hardening_.
...
- **Q1/Q-A6 — Deployment target:** **Backend = Supabase Cloud (managed); Frontend = AWS/GCP/Azure
  ...
  ships the static React build to the chosen cloud's CDN — _no Vercel preview deploys; preview
  environments come from the chosen cloud or Supabase branch previews instead._

### Sizing reality

Full-surface replication at production quality — _with the test coverage Lovable never had_
...
CI green · Google SSO login · **tenant isolation verified by automated tests** · _(extended by
Q-A1)_ full-surface parity with `lovable-source` confirmed against `types.ts`.
