...
| --------- | --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **F1 ✅** | Terraform skeleton + remote state | **Landed.** `infra/terraform/bootstrap/` (remote state S3+DynamoDB, GitHub OIDC provider + per-env least-privilege deploy roles), `.github/workflows/terraform.yml` (fmt+validate on PR; plan gated on `AWS_BOOTSTRAP_ROLE_ARN`; never applies). `terraform validate` + `fmt` pass locally (provider via releases mirror).                                                                                                                                                    | ✅ `terraform validate` green; zero static AWS keys; no infra provisioned.                                                                                              |
| **F2 ✅** | Frontend infra module (per env)   | **Landed.** `modules/frontend` (private versioned S3 + CloudFront/OAC, ACM us-east-1 with `*.<zone>` wildcard, security headers, SPA 403/404→index.html, Route53 apex+wildcard alias, OAC-only bucket policy) + `envs/{dev,staging,production}` instantiations. CI `validate` matrix covers all roots. `terraform validate` Success on all three envs locally; `fmt` clean. **`apply` still pending a real account (Q-F1).**                                                  | ✅ `terraform validate` green per env; module + 3 env roots parse against the real AWS provider schema. Live `apply`/TLS/subdomain checks happen at first apply (Q-F1). |
| **F3 ✅** | Frontend deploy pipeline          | **Landed.** `deploy-staging.yml` (merge to `main` → build → S3 sync → CloudFront invalidation via the F1 OIDC role) + `deploy-production.yml` (`v*` tag → same, gated by the `production` GitHub Environment's required-reviewer rule; prod OIDC trust restricted to that environment). Per-env `VITE_*` from GitHub Environment vars/secrets; jobs `if AWS_DEPLOY_ROLE_ARN != ''` so inert until provisioned. `docs/runbooks/deploy.md` documents the per-env setup + flows. | ✅ workflows valid + inert pre-provisioning; cache/invalidation correct; no long-lived keys. Live deploy verified at first apply (Q-F1).                                |
