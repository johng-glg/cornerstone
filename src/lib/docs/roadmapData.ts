...
export const FUTURE_BUILDS: RoadmapItem[] = [
  ...
  // Payments
  {
    ...
    name: 'Payment Processor Integration (Forth Pay)',
    category: 'Payments',
    priority: 'High',
    description: 'Forth Pay integration for client registration, draft transaction lifecycle, and status polling.',
    status: 'Completed',
    difficulty: 4,
    benefit: 5,
    notes: 'Implemented via 8 edge functions: forth-auth (OAuth token cache), forth-register-client (background registration during enrollment), forth-sync-client (pull contact data), forth-create-draft / forth-update-draft / forth-cancel-draft (draft lifecycle), forth-pause-resume (service pause/resume mirroring), forth-poll-transactions (scheduled status polling open → pending → cleared/cancelled). Client-side useForthApi hook wired into EnrollmentWizard, ServiceDetailSheet, and TransactionDetailSheet. forth_crm_id stored on clients/services. FORTH_API_KEY and FORTH_CLIENT_ID configured.',
  },
  {
    id: 'forth-webhook-receiver',
    name: 'Forth Webhook Receiver',
    category: 'Payments',
    priority: 'Medium',
    description: 'Inbound webhook endpoint for Forth → Lovable status pushes to reduce reliance on polling.',
    status: 'Planned',
    difficulty: 2,
    benefit: 4,
    notes: 'Today everything is polled outbound via forth-poll-transactions. A webhook receiver would deliver near-real-time transaction status updates and reduce API call volume.',
  },
  {
    id: 'forth-reconciliation',
    name: 'Forth Reconciliation & Dispute UI',
    category: 'Payments',
    priority: 'Medium',
    description: 'Reconciliation dashboard for mismatched transactions and a UI for handling chargebacks/disputes.',
    status: 'Planned',
    difficulty: 3,
    benefit: 4,
    notes: 'No reconciliation view or dispute workflow currently exists. Needed for month-end close and chargeback handling.',
  },
...
export const INTEGRATIONS: IntegrationItem[] = [
  ...
  },
  {
    id: 'entrust-united',
    name: 'Entrust United',
    purpose: 'Payment processor (future replacement for Forth Pay)',
    priority: 'Medium',
    status: 'Research',
    notes: 'Planned future processor. Will replace Forth Pay. Need API documentation, sandbox access, and webhook setup.',
  },
