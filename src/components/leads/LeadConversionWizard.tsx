    ...
    try {
      ...
      const { data: engagement, error: engagementError } = await supabase
        ...
        .insert([{
          ...
          engagement_number: '', // Auto-generated by trigger
        }] as any)
        .select()
        .single();
      ...
      // 4. Link engagement to contact
      await supabase.from('engagement_contacts').insert([{
      ...
      for (const serviceId of serviceData.services) {
        await supabase.from('engagement_services').insert([{
          engagement_id: engagement.id,
  ...
  return (
    <Dialog open={!!leadId} onOpenChange={() => onClose()}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="font-heading text-xl">CONVERT LEAD TO ENGAGEMENT</DialogTitle>
          <DialogDescription>
            Complete the following steps to convert this lead into an active engagement.
          </DialogDescription>
        </DialogHeader>
